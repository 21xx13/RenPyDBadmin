...
$data_script = parse_script($script);
$label = $data_script['label'];
$message = $data_script['task'];
$options = $data_script['options'];
...
DBi::$conn->query("UPDATE `task_info` SET `task_text` = '$message', `task_name` = '$title', `label` = '$label', `game_script` = '$script' WHERE `id` = $id");
